...
* **Pros:** Extremely fast read/write operations (ideal for WordPress caching plugins or native PHP sessions).
* **Cons:** **All data is permanently lost when the container is restarted or stopped.**

> [!WARNING]
> If your WordPress application relies on storing user session files or temporary media generations on disk in `/tmp`, be aware that a `make restart` will instantly log out all currently active users (if using PHP sessions). If you need session persistence across deployments, consider enabling the [Redis Session Backend](redis.md) instead.
